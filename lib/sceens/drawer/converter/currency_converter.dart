...
class CurrencyConverterScreen extends StatefulWidget {
  const CurrencyConverterScreen ({super.key, required this.account, required this.expenseData});
...
class _CurrencyConverterScreenState extends State<CurrencyConverterScreen > {
  ...
  @override
  void initState() {
    fromCurrency = listCurrency.firstWhere((element) => element.code == widget.account.currencyCode);
    toCurrency = listCurrency.firstWhere((element) => element.code != widget.account.currencyCode);
  ...
  void convert() async {
    if ( formKey.currentState!.validate()) {
      ...
      await converter.getConvert(fromCurrency.code, toCurrency.code, double.parse(fromCurrencyctrl.text)).then((value) {
        setState(() {
        ...
        child: Center(
          child: Column(
            ...
            children: [
              Column(
                children: [
                  const Text('Công cụ chuyển đổi tiền tệ', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w700, fontSize: 24),),
                  const SizedBox(height: 8,),
                  Text('Chuyển đổi các đơn vị tiền tệ có trong hệ thống và kiểm tra tỉ giá.', style: TextStyle(color: Colors.grey[600]), textAlign: TextAlign.center,),
                  const SizedBox(height: 8,),
                ],
              ),
              Card(
                    ...
                    children: [
                              ...
                              Padding(
                                ...
                                child: Column(
                                  children: [
                                    DropdownButton(
                                      value: fromCurrency,
                                      items: List.generate(listCurrency.length, (index) => DropdownMenuItem(
                                    ...
                                    Form(
                                      ...
                                      child: TextFormField(
                                        ...
                                        onSaved: (value){
                                          if(currenciesCodeHasDecimal.contains(fromCurrency.code)){
                                              fromCurrencyctrl.text = value!.trim();
                                            }else{
                                              fromCurrencyctrl.text = value!.split('.')[0];
                                            }
                                        },
                          ...
                          Positioned.fill(
                            child: Align(
                              ...
                              child: Container(
                                ...
                                child: const Icon(EvaIcons.arrowDownwardOutline,color: Colors.white,),
                              ),
                            ),
                          )
                        ],
                      ...
                      ElevatedButton(
                        ...
                        child: isSubmited 
                          ...
                          : const Text('Chuyển đổi', style: TextStyle(color: Colors.black, fontSize: 18,fontWeight: FontWeight.normal),))
                    ],
                  ),
                ),
              ),
              const SizedBox(height: 16,),
              Text('Tỉ giá', style: TextStyle(color: Colors.grey[600],),),
              const SizedBox(height: 8,),
              rate != null
                ? Text('1 ${fromCurrency.code} = $rate ${toCurrency.code}', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w500),)
                : const Center(child: CircularProgressIndicator(),)
            ],
          ),
        ),
